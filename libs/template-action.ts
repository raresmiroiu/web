...
export async function createTemplateAction(
  ...
  if (!name || name.trim() === "") {
    return {
      success: false,
      message: "Numele template-ului este obligatoriu.",
    };
  }
  ...
  try {
    ...
    if (!orgId)
      return { success: false, message: "Organizația nu a fost găsită." };
    ...
    return {
      success: false,
      message: "A apărut o eroare la salvarea template-ului.",
    };
  }
}
...
export async function deleteTemplateAction(
  ...
  try {
    ...
    await pool.query("DELETE FROM templates WHERE id = $1 AND org_id = $2", [
      templateId,
      orgId,
    ]);
